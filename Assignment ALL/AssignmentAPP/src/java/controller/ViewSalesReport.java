    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            for (Transactions transaction : transactionList) {
                totalPayment += transaction.getPaymentId().getTotal();
            }
            HttpSession session = request.getSession();
            session.setAttribute("totalPayment", totalPayment);
            session.setAttribute("transactionList", transactionList);
            response.sendRedirect("admin/ViewSalesReport.jsp");
        } catch (Exception ex) {
            Logger.getLogger(ViewSalesReport.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
