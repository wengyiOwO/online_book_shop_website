 ...
 * @author yongy
 */
public class ViewLeastSalesReport extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<Map.Entry<String, Integer>> least5 = list.subList(0, 5);
            HttpSession session = request.getSession();
            session.setAttribute("least5", least5);
            session.setAttribute("bookList", bookList);
            response.sendRedirect("admin/ViewLeastSalesReport.jsp");
        } catch (Exception ex) {
            Logger.getLogger(ViewLeastSalesReport.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
